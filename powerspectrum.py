...
POWER_SPECTRUM_DIRECTION = "upward"
...
POWER_SPECTRUM_LABELS = {
    "highfre axion+backreaction": r"$\left|300\right\rangle\rightarrow\left|322\right\rangle$",
    "highfre644 axion+backreaction": r"$\left|544\right\rangle\rightarrow\left|644\right\rangle$",
...
EXPORT_TARGETS = (
    ExportTarget(
        ...
        path=FREQUENCY_DATA_DIR / "highfre322_axion_backreaction_total_frequency_amplitude.txt",
        builder_name="highfre",
    ),
    ExportTarget(
        label="highfre644 axion+backreaction",
        path=FREQUENCY_DATA_DIR / "highfre644_axion_backreaction_total_frequency_amplitude.txt",
...
def build_highfre_exports():
    simulator = highfre322.EccentricResonantTidalGA(
        M_bh=1.0,
        M_star=0.01,
...
def build_highfre644_exports():
    simulator = highfre644.EccentricResonantTidalGA(
        M_bh=1.0,
        M_star=0.01,
        alpha=EXPECTED_HIGHFREQ_ALPHA,
        bh_spin=EXPECTED_HIGHFREQ_BH_SPIN,
...
def plot_omega_curves(curves, sensitivity_curves):
    ...
    pdf_path = FIGURE_DIR / f"sgwb_power_spectra_with_sensitivity{SGWB_OUTPUT_SUFFIX}_upward.pdf"
    try:
