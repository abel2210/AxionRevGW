...
def write_sweep_outputs(points: list[SweepPoint], finite_results) -> None:
    ...
    lines = [
        ...
        "",
        r"- the analytic \(C_{\rm out}(z_{\rm LZ})\) curve;",
        r"- controlled fixed-normalization sweep-rate points;",
        r"- full event-local physical points from the \(q=10^{-3}\), \(|644\rangle\rightarrow|544\rangle\) family.",
        "",
    ...
    lines.extend(
        [
            ...
            "## Figures",
            "",
            rf"- finite \(z_{{\rm LZ}}\): `{FINITE_Z_FIGURE.as_posix()}`",
            f"- finite envelope: `{FINITE_ENV_FIGURE.as_posix()}`",
            rf"- sweep \(z_{{\rm LZ}}\): `{SWEEP_Z_FIGURE.as_posix()}`",
            f"- sweep envelope: `{SWEEP_ENV_FIGURE.as_posix()}`",
            f"- combined four-panel figure: `{COMBINED_FIGURE.as_posix()}`",
        ]
    )
...
def plot_finite_envelope(ax, finite_results, show_title=True):
    ...
    ax.axvline(LZ_WINDOW_WIDTHS, color="0.5", lw=0.6, ls="--")
    ax.set_xlim(-LZ_WINDOW_WIDTHS, LZ_WINDOW_WIDTHS + 12.0)
...
def save_combined(alpha_grid, z_grid, c_grid, finite_results, points, envelopes):
    cm = 1.0 / 2.54
    fig, axes = plt.subplots(2, 2, figsize=(8.6 * cm, 8.4 * cm), constrained_layout=False)

    plot_finite_z(axes[0, 0], alpha_grid, z_grid, c_grid, finite_results, show_title=True)
    plot_finite_envelope(axes[0, 1], finite_results, show_title=True)
    plot_sweep_z(axes[1, 0], points, show_title=True)
