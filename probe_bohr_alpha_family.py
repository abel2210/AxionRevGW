...
def write_outputs(
    ...
    for item in full_results:
        interpretation = "adiabatic branch with negligible outgoing coherence" if item.alpha < 0.24 else "finite-coherence branch"
        lines.append(
    ...
    if alpha_018 is not None and alpha_030 is not None:
        ...
        lines.extend(
            [
                f"- The `alpha=0.18` full run gives `C_post={alpha_018.c_post_num:.3e}`, consistent with negligible outgoing transition coherence on the adiabatic branch.",
    ...
    lines.extend(
        [
            "",
            f"CSV table: `{CSV_PATH.relative_to(BASE_DIR).as_posix()}`",
            f"Figure: `{FIGURE_PATH.relative_to(BASE_DIR).as_posix()}`",
        ]
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the q=0.01 Bohr alpha-family outgoing-coherence validation.")
    parser.add_argument(
        "--full-alphas",
        nargs="*",
        type=float,
