...
def build_pure_peters_frequency_domain(
    *,
    ...
    metadata = {
        ...
        "sample_points": f"{int(sample_points)}",
        "pad_factor": f"{int(pad_factor)}",
        "fft_nyquist_hz": f"{freq_hz[-1]:.16e}",
        "fft_dt_obs_s": f"{dt_obs:.16e}",
        "fft_df_hz": f"{freq_hz[1] - freq_hz[0]:.16e}",
        "fft_n_samples": f"{len(t_obs)}",
        "fft_n_fft": f"{n_fft}",
        "fft_window": "kaiser",
        "fft_window_beta": f"{float(window_beta):.16e}",
        "fft_mean_removed": "1",
